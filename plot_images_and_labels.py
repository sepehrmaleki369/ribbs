...
def plot_images_and_labels():
    """Plot Approach 2 dataset: images with their ground truth labels only"""
    ...
    os.makedirs("images_and_labels", exist_ok=True)
    ...
    for i, (img_file, label_file) in enumerate(zip(image_files, label_files)):
        try:
            ...
            print(f"Loaded {img_file}: Image {image.shape}, Label {label.shape} (classes: {np.unique(label)})")
    ...
    fig, axes = plt.subplots(rows, cols * 2, figsize=(cols * 10, rows * 5))
    fig.suptitle('Images and Their Ground Truth Labels', fontsize=16)
    ...
    plt.tight_layout()
    plt.savefig('images_and_labels/all_samples_overview.png', dpi=300, bbox_inches='tight')
    print("All samples overview saved as: images_and_labels/all_samples_overview.png")
    
    # Create detailed analysis for first few samples
    print("\n=== CREATING DETAILED ANALYSIS ===")
    
    num_detailed = min(8, len(all_data))
    
    fig, axes = plt.subplots(num_detailed, 3, figsize=(15, 5 * num_detailed))
    fig.suptitle('Detailed Analysis: First 8 Samples', fontsize=16)
    ...
    for i in range(num_detailed):
        ...
        # Histogram
        axes[i, 2].hist(data['label'].flatten(), bins=20, alpha=0.7, color='blue')
        axes[i, 2].set_title(f"Sample {i+1}: Label Distribution")
        axes[i, 2].set_xlabel('Class Value')
        axes[i, 2].set_ylabel('Frequency')
    
    plt.tight_layout()
    plt.savefig('images_and_labels/detailed_analysis.png', dpi=300, bbox_inches='tight')
    print("Detailed analysis saved as: images_and_labels/detailed_analysis.png")
    
    # Create overlay visualization
    print("\n=== CREATING OVERLAY VISUALIZATION ===")
    
    fig, axes = plt.subplots(2, 4, figsize=(20, 10))
    fig.suptitle('Image-Label Overlay Analysis', fontsize=16)
    ...
    for i in range(min(8, len(all_data))):
        ...
        # Create overlay
        image_normalized = data['image'] / 255.0
        label_normalized = data['label'] / data['label'].max()  # Normalize to [0,1]
        
        # Create RGB overlay
        overlay = image_normalized.copy()
        overlay[:, :, 0] = np.clip(overlay[:, :, 0] + label_normalized * 0.5, 0, 1)  # Add red channel
        
        axes[row, col].imshow(overlay)
        axes[row, col].set_title(f"Sample {i+1}: Overlay\nClasses: {data['label_stats']['unique_values']}")
        axes[row, col].axis('off')
    
    plt.tight_layout()
    plt.savefig('images_and_labels/overlay_analysis.png', dpi=300, bbox_inches='tight')
    print("Overlay analysis saved as: images_and_labels/overlay_analysis.png")
    ...
    axes[1, 0].bar(range(len(label_means)), label_means, alpha=0.7)
    axes[1, 0].set_title('Label Mean Values')
    axes[1, 0].set_xlabel('Sample Index')
    axes[1, 0].set_ylabel('Mean Label Value')
    ...
    axes[1, 1].set_title('Class Frequency Across Samples')
    axes[1, 1].set_xlabel('Class Value')
    axes[1, 1].set_ylabel('Number of Samples')
    
    plt.tight_layout()
    plt.savefig('images_and_labels/statistics_summary.png', dpi=300, bbox_inches='tight')
    print("Statistics summary saved as: images_and_labels/statistics_summary.png")
    ...
    print(f"\nAll visualization files saved to: images_and_labels/")
