...
class GeneralizedDataset(Dataset):
    ...
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        # Check for required modalities - be flexible about target modality
        if 'image' not in self.modality_files:
            raise ValueError("Split must define 'image' modality in split_cfg.")
        
        # Check for target modality (could be 'label', 'distance_map', etc.)
        target_modalities = [mod for mod in self.modality_files.keys() if mod != 'image']
        if not target_modalities:
            raise ValueError("Split must define at least one target modality (e.g., 'label', 'distance_map') in split_cfg.")
        
        # Use the first target modality as the main target
        self.target_modality = target_modalities[0]
        
        # Ensure all modalities have the same number of files
        for modality in self.modality_files:
            if len(self.modality_files[modality]) != len(self.modality_files['image']):
                raise ValueError(f"All modalities must have the same number of files. "
                              f"Image: {len(self.modality_files['image'])}, "
                              f"{modality}: {len(self.modality_files[modality])}")
        ...
        if self.save_computed:
            for key in [m for m in self.modalities if m not in ['image', self.target_modality]]:
                logger.info(f"Generating {key} modality maps...")
                for file_idx, _ in tqdm(list(enumerate(self.modality_files[self.target_modality])),
                                        total=len(self.modality_files[self.target_modality]),
                                        desc=f"Processing {key} maps"):
                    lbl = load_array_from_file(self.modality_files[self.target_modality][file_idx])
    ...
    def _postprocess_patch(self, data: Dict[str, np.ndarray], sample_rng: np.random.Generator) -> Dict[str, np.ndarray]:
        ...
        for k, arr in list(data.items()):
            ...
            elif arr.ndim == 3 and self.data_dim == 2:
                # For 2D data, ensure channels are in the first dimension
                if k == 'image':
                    # Image: (H, W, C) -> (C, H, W)
                    data[k] = arr.transpose(2, 0, 1)
                else:
                    # Label: (H, W) -> (1, H, W) (already handled above)
                    pass
