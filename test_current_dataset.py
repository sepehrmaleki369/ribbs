...
# Verify what the current dataset is loading
...
def test_current_dataset():
    """Test the current dataset to see what it's loading"""
    ...
    create_test_visualization(sample)
    
    print(f"\n✅ Current dataset test completed!")

def create_test_visualization(sample):
    """Create a test visualization of the sample"""
    ...
    axes[1].set_title('Distance Map (Current)')
    axes[1].axis('off')
    plt.colorbar(im, ax=axes[1], fraction=0.046, pad=0.04)
    
    # Distance map histogram
    axes[2].hist(distance_map.flatten(), bins=50, alpha=0.7)
    axes[2].set_title('Distance Map Distribution')
    axes[2].set_xlabel('Distance Value')
    axes[2].set_ylabel('Frequency')
    
    plt.tight_layout()
    plt.savefig('predictions/current_dataset_test.png', dpi=300, bbox_inches='tight')
    print(f"Test visualization saved as predictions/current_dataset_test.png")
