...
def load_array_from_file(file_path: str) -> Optional[np.ndarray]:
    ...
    Load an array from disk. Supports:
      - .npy      → numpy.load
      - .tif/.tiff → rasterio.open + read (transposed to HWC format)
      - .gif      → PIL.Image.open + numpy.array
    Returns:
    ...
    loaders = {
        ...
        '.tif': lambda p: rasterio.open(p).read().astype(np.float32).transpose(1, 2, 0),
        '.tiff': lambda p: rasterio.open(p).read().astype(np.float32).transpose(1, 2, 0),
        '.gif': lambda p: np.array(Image.open(p)),
    }
